...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    pdet->detect(img, img_undist, center, radius);
    ROS_INFO(" Object detecting center is %f %f   r if %f",
                pdet->center.x, pdet->center.y, pdet->radius);

    center.x = pdet->center.x;
    center.y = pdet->center.y;
    radius = pdet->radius;
    center.y = 0;

    if(center.x !=0 || center.y != 0 )
    {
        //ROS_INFO(" R is %f  r is %f",
        //        obj_R, pdet->center.y, pdet->radius);

        ptracker->track(center, radius, obj_R);
        p_w = ptracker->p_w;
        if(p_w.x != 0 || p_w.y != 0 || p_w.z != 0 )
            ROS_INFO(" Object in space is %f %f %f",
                p_w.x, p_w.y, p_w.z);
    }
}
...
int main(int argc, char **argv) 
{
    ...
    ros::NodeHandle nh("~");
    nh.getParam("camera_matrix_add", camera_matrix_add);
    nh.getParam("json_add", json_add);
    nh.getParam("json_file_add", json_file_add);
    nh.getParam("obj_R", obj_R);
    ...
    if(pdet->initialize(camera_matrix_add,color_begin, color_end, camera_matrix))
    {
        ROS_INFO("Detector is initialized, start detecting obj...");
        //cout << camera_matrix << endl;
        ptracker->set_camera(camera_matrix);
        ROS_INFO("Tracker is initialized, start tracking obj...");
    }
    else
    {
        return EXIT_FAILURE;
